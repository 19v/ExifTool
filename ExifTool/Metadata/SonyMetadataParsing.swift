...
nonisolated enum SonyMetadataExtractor {
    ...
    private static func ifdEntry(forTag targetTag: UInt16, in data: Data, tiffStart: Int, ifdOffset: Int, endian: TIFFEndian) -> TIFFIFDEntry? {
        ...
        guard entryCount > 0, entryCount < 512, absoluteIFDOffset + 2 + entryCount * 12 <= data.count else {
            return nil
        }
...
nonisolated enum SonyMakerNoteParser {
    ...
    private static func parseIFD(in data: Data, offset: Int, endian: Endian) -> [IFDEntry] {
        ...
        for index in 0..<count {
            ...
            guard valueCount > 0, valueCount <= 8192 else {
                continue
            }
    ...
    private static func valueData(in data: Data, entryOffset: Int, byteCount: Int, ifdOffset: Int, endian: Endian) -> Data? {
        guard byteCount > 0 else {
            return nil
        }
